...
def transform_annotations_to_kitti_format_for_nuscenes(annos, map_name_to_kitti=None, info_with_fakelidar=False):
    """
    Args:
        annos:
        map_name_to_kitti: dict, map name to KITTI names (Car, Pedestrian, Cyclist)
    ...
    for anno in annos:
        ...
        if len(gt_boxes_lidar) > 0:
            ...
            anno['rotation_y'] = -gt_boxes_lidar[:, 6] - np.pi / 2.0
            anno['alpha'] = -np.arctan2(-gt_boxes_lidar[:, 1], gt_boxes_lidar[:, 0]) + anno['rotation_y']
            # set difficulty based on distance
            distance = np.linalg.norm(anno['location'], axis=1)
            anno['occluded'][distance>25] = 1
            anno['occluded'][distance>40] = 2
